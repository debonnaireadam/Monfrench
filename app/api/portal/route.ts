...
export async function POST(request: Request) {
  try {
    ...
    if (action === "get_progress" || action === "save_progress") {
      ...
      const work=await env.DB.prepare(`SELECT id,status,state_r2_key,state_size,state_version,saved_at FROM student_work WHERE assignment_id=? AND student_id=?`).bind(assignmentId,current.id).first<{id:string;status:string;state_r2_key:string|null;state_size:number;state_version:number;saved_at:string|null}>();
      ...
      if(work?.status==="submitted"||work?.status==="locked")return json({error:"Ce travail a déjà été remis. Votre professeur doit le rouvrir avant toute modification."},409);
      const stamp=now(),stateKey=`student-work/${assignmentId}/${current.id}/state.json`,stateVersion=Math.max((work?.state_version??0)+1,Number(envelope.revision)||0);
      await env.FILES.put(stateKey,serialized,{httpMetadata:{contentType:"application/json; charset=utf-8"}});
      await env.DB.batch([env.DB.prepare(`INSERT INTO student_work(id,assignment_id,student_id,status,state_r2_key,state_size,state_version,saved_at,created_at,updated_at) VALUES(?,?,?,'draft',?,?,?,?,?,?) ON CONFLICT(assignment_id,student_id) DO UPDATE SET status='draft',state_r2_key=excluded.state_r2_key,state_size=excluded.state_size,state_version=excluded.state_version,saved_at=excluded.saved_at,updated_at=excluded.updated_at`).bind(work?.id??id(),assignmentId,current.id,stateKey,stateSize,stateVersion,stamp,stamp,stamp),env.DB.prepare(`UPDATE assignment_students SET status=CASE WHEN status='assigned' THEN 'opened' ELSE status END,opened_at=COALESCE(opened_at,?) WHERE assignment_id=? AND student_id=?`).bind(stamp,assignmentId,current.id)]);
