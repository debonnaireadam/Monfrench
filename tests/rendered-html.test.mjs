...
test("build contains the portal and protected API routes", async () => {
  ...
  assert.match(page, /function ActivityFullscreen/);
  ...
  assert.doesNotMatch(page, /homework-view-controls/);
  ...
  assert.match(page, /homework-design-settings/);
  assert.match(page, /monfrench-homework-preferences/);
  assert.match(page, />Minimal<\/option>/);
...
test("private files require ownership or assignment access", async () => {
  ...
  assert.match(route, /marker\.index < lastScript/);
});
