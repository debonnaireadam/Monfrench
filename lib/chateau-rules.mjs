...
export function hashPassword(password,{salt=randomBytes(16).toString("hex"),pepper=""}={}){
  if(typeof password!=="string"||password.length<1)throw new Error("password_required");
...
export function validateBridgeMessage(value){
  ...
  let serialized;try{serialized=JSON.stringify(value)}catch{return {ok:false,error:"invalid_payload"}}if(serialized.length>1_000_000)return {ok:false,error:"payload_too_large"};
