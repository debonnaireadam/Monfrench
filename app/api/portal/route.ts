...
type User = { id: string; username: string; display_name: string; role: Role; must_change_password: number };
...
async function staffDashboard(current: Session) {
  ...
  const [students, teachers, folders, activities, assignments, submissions, auditLogs, storage] = await Promise.all([
    ...
    (owner ? env.DB.prepare(`SELECT u.id,u.username,u.display_name,u.role,u.active,u.must_change_password,u.created_at,u.deactivated_at,(SELECT COUNT(*) FROM teacher_students ts WHERE ts.teacher_id=u.id) student_count FROM users u WHERE u.role IN ('owner','teacher') AND u.trashed_at IS NULL ORDER BY u.role,u.display_name`).all() : Promise.resolve({ results: [] })),
    ...
    (owner ? env.DB.prepare(`SELECT s.*,u.display_name student_name,COALESCE(v.title,ac.title) title,a.created_by teacher_id,t.display_name teacher_name FROM submissions s JOIN users u ON u.id=s.student_id JOIN assignments a ON a.id=s.assignment_id JOIN users t ON t.id=a.created_by JOIN activities ac ON ac.id=a.activity_id LEFT JOIN activity_versions v ON v.id=a.activity_version_id WHERE s.trashed_at IS NULL ORDER BY s.submitted_at DESC`) : env.DB.prepare(`SELECT s.*,u.display_name student_name,COALESCE(v.title,ac.title) title,a.created_by teacher_id FROM submissions s JOIN users u ON u.id=s.student_id JOIN teacher_students ts ON ts.student_id=s.student_id JOIN assignments a ON a.id=s.assignment_id JOIN activities ac ON ac.id=a.activity_id LEFT JOIN activity_versions v ON v.id=a.activity_version_id WHERE ts.teacher_id=? AND s.trashed_at IS NULL ORDER BY s.submitted_at DESC`).bind(current.id)).all(),
...
async function duplicateItems(current: Session, kind: string, recordIds: string[]) {
  ...
  for (const recordId of recordIds) {
    if (kind === "activity") {
      ...
      const activityId = id(), versionId = id(), stamp = now(), title = `${source.title} — copie`;
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO activities(id,title,category,description,instructions,r2_key,original_name,content_type,external_url,current_version_id,folder_id,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(activityId,title,source.category,source.description,source.instructions,source.version_r2_key ?? source.r2_key ?? null,source.version_original_name ?? source.original_name ?? null,source.version_content_type ?? source.content_type ?? null,source.version_external_url ?? source.external_url ?? null,versionId,source.folder_id ?? null,current.id,stamp),
...
export async function POST(request: Request) {
  try {
    ...
    if (!staff(current) && !["open_assignment","submit"].includes(action)) return json({error:"Réservé à l’enseignant."},403);
    ...
    if(action==="create_activity"){
      ...
      const description=String(get(data,"description")??""),instructions=String(get(data,"instructions")??""),runtimeKind=["glassbook","pellucide"].includes(String(get(data,"runtimeKind")))?String(get(data,"runtimeKind")):"generic";
      try{await env.DB.batch([env.DB.prepare(`INSERT INTO activities(id,title,category,description,instructions,r2_key,original_name,content_type,external_url,current_version_id,folder_id,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(activityId,title,category,description,instructions,r2Key,originalName,contentType,externalUrl,versionId,folderId,current.id,stamp),env.DB.prepare(`INSERT INTO activity_versions(id,activity_id,version_number,title,category,description,instructions,r2_key,original_name,content_type,external_url,file_size,runtime_kind,online_capable,manifest_version,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,0,1,?,?)`).bind(versionId,activityId,1,title,category,description,instructions,r2Key,originalName,contentType,externalUrl,fileSize,runtimeKind,current.id,stamp)]);}catch(error){if(r2Key)await env.FILES.delete(r2Key);throw error;}await audit(current,action,"activity",activityId);return json({ok:true,activityId});
    }
    ...
    if(action==="complete_activity_upload"){
      ...
      const versionId=String(grant.version_id),stamp=now(),title=String(grant.title),category=String(grant.category),description=String(grant.description??""),instructions=String(grant.instructions??""),runtimeKind=["glassbook","pellucide"].includes(String(get(data,"runtimeKind")))?String(get(data,"runtimeKind")):"generic",folderId=grant.folder_id?String(grant.folder_id):null,originalName=String(grant.original_name);
      try{await env.DB.batch([env.DB.prepare(`INSERT INTO activities(id,title,category,description,instructions,r2_key,original_name,content_type,current_version_id,folder_id,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`).bind(activityId,title,category,description,instructions,key,originalName,contentType,versionId,folderId,current.id,stamp),env.DB.prepare(`INSERT INTO activity_versions(id,activity_id,version_number,title,category,description,instructions,r2_key,original_name,content_type,file_size,runtime_kind,online_capable,manifest_version,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,0,1,?,?)`).bind(versionId,activityId,1,title,category,description,instructions,key,originalName,contentType,fileSize,runtimeKind,current.id,stamp),env.DB.prepare(`UPDATE upload_sessions SET status='completed' WHERE id=? AND status='assembling'`).bind(uploadId)]);}catch(error){await env.FILES.delete(key);await env.DB.prepare(`UPDATE upload_sessions SET status='uploading' WHERE id=? AND status='assembling'`).bind(uploadId).run();throw error;}
      try{await deleteStagedUpload(prefix);}catch{}await audit(current,action,"activity",activityId);return json({ok:true,activityId});
    }
    ...
    if(action==="assign"){
      ...
      const createdIds:string[]=[],stamp=now();for(const activityId of activityIds){const activity=await env.DB.prepare(`SELECT a.current_version_id FROM activities a JOIN activity_versions v ON v.id=a.current_version_id AND v.activity_id=a.id WHERE a.id=? AND a.trashed_at IS NULL AND a.archived_at IS NULL`).bind(activityId).first<{current_version_id:string}>();if(!activity?.current_version_id)return json({error:"Activité ou version introuvable."},404);const assignmentId=id();await env.DB.batch([env.DB.prepare(`INSERT INTO assignments(id,activity_id,activity_version_id,folder_id,due_at,instructions,status,published_at,created_by) VALUES(?,?,?,?,?,?,'published',?,?)`).bind(assignmentId,activityId,activity.current_version_id,folderId,String(get(data,"dueAt")??"")||null,String(get(data,"instructions")??""),stamp,current.id),...studentIds.map(studentId=>env.DB.prepare(`INSERT INTO assignment_students(assignment_id,student_id,status,assigned_at) VALUES(?,?,'assigned',?)`).bind(assignmentId,studentId,stamp))]);createdIds.push(assignmentId);}await audit(current,action,"assignment",null,{activityIds,studentIds,createdIds,folderId});return json({ok:true,createdIds});
    }
