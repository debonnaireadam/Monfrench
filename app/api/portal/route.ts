...
export async function POST(request: Request) {
  try {
    ...
    if (action === "get_progress" || action === "save_progress") {
      ...
      const envelopeApp=String(envelope?.app??""),expectedSchema=`${envelopeApp}.student-state`;
      if(!envelope||!["glassbook","pellucide"].includes(envelopeApp)||envelope.schema!==expectedSchema||Number(envelope.schemaVersion)!==1||typeof envelope.state!=="object"||envelope.state===null)return json({error:"État d’activité invalide."},400);
    ...
    if(action==="create_activity"){
      ...
      const description=String(get(data,"description")??""),instructions=String(get(data,"instructions")??""),runtimeKind=["glassbook","pellucide"].includes(String(get(data,"runtimeKind")))?String(get(data,"runtimeKind")):"generic",onlineCapable=["glassbook","pellucide"].includes(runtimeKind)?1:0;
