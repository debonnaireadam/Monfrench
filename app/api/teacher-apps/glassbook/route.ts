...
export async function GET(request: Request) {
  ...
  if (!token) return new Response("Non autorisé", { status: 401, headers: noStore });
  const user = await env.DB.prepare(`SELECT u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id_hash=? AND s.expires_at>? AND s.created_at>? AND u.active=1`).bind(await hash(token), new Date().toISOString(), sessionCutoff()).first<{ role: string }>();
  if (!user) return new Response("Non autorisé", { status: 401, headers: noStore });
  if (user.role !== "teacher") return new Response("Réservé à l’enseignant", { status: 403, headers: noStore });
